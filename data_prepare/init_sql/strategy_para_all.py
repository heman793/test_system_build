...
class Strategy_para(ConfigCommon):
    ...
    def clear_db_strategy(self, date):
        ...
        delete_table_list = ['strategy_parameter']
        map(lambda table: sql_list.append(delete_sql % (table, date,
                                                        strategy_name_var)),
            delete_table_list)
        self.execute_db_sql(db='strategy', sql_list=sql_list)
        # print sql_list

    def insert_atp_strategy_para(self):
        filename = 'strategy_parameter.csv'
        strategy_file_path = os.path.join(atp_strategy_para_path, filename)
