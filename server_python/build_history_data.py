...
def start_build_quotes_base(ctp_file_name):
    if ctp_file_name is None:
        ctp_file_name = filter_market_file(market_file_path)
...
def build_history_data():
    ...
    build_ticker_exchange()
    build_trading_time_dict()
    ctp_file_path = filter_market_file(market_file_path)
    ...
    for (ticker, quote_data_list) in quote_dict.items():
        ...
        quote_bar_list = __build_minbar_data(ticker, quote_data_list)
...
if __name__ == '__main__':
    ...
    start(ctp_file_name)
    # start_build_quotes_base('CTP_Market_2017-01-20_2.txt')