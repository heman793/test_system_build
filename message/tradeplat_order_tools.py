# -*- coding: utf-8 -*-
from tradeplat_message_tools import *
import datetime
import bcl_pb2
...
def __recv_order_info(socket, last_update=None):
    instrument_dict, market_dict = send_instrument_info_request_msg(socket)
    order_msg_list = send_order_info_request_msg2(socket,last_update)
    ...
    for order_info in order_msg_list:
        ...
        order_view.client_id = order_type_dict[str(order_info.Order.CliOrdID)]
        order_view_list.append(order_view)

    order_view_list.sort(key=lambda obj: obj.transaction_time, reverse=True)
    print order_view.order_account
    print order_view_list
    return order_view_list


def reject_order_query():
    socket = socket_init()
...
def __query_none_order(socket):
    ...
    none_order_list = []
    for order_view in order_view_list:
        if order_view.order_status == 'Filled' :
...
def __query_client_order(socket):
    order_view_list = __recv_order_info(socket)

    client_order_list = []
    for order_view in order_view_list:
        if order_view.ticker == '600000':
            client_order_list.append(order_view)
    # print order_view
    return client_order_list


# 批量取消状态为none的订单
def none_order_cancel_tools(limit_time=None):
    socket = socket_init()
...
def query_none_order():
    socket = socket_init()
    order_view_list = __recv_order_info(socket)

    none_order_list = []
    for order_view in order_view_list:
        if order_view.operation_status == 'Filled' :
            creation_time_str = order_view.creation_time.strftime('%Y-%m-%d %H:%M:%S')
            now_time_str = date_utils.get_today_str('%Y-%m-%d %H:%M:%S')
            interval_seconds = date_utils.get_interval_seconds(creation_time_str, now_time_str)
...
# 批量取消状态为none的订单
def none_order_cancel_tools2(fund_name=None):
    socket = socket_init()
    order_view_list = __recv_order_info(socket)
...
def query_order(query_ticker):
    ...
    for order_view in order_view_list:
        if order_view.ticker == query_ticker:
            ...
            print order_view.order_status

def query_order_by_clientid(clientid, last_update):
    socket = socket_init()
    order_list = __recv_order_info(socket, last_update)
    print order_list
    order_client_list = []
...
if __name__ == '__main__':
    ...
    # query_order('test5',600000)
    # __recv_order_info(socket)
    reject_order_query()
    # query_order_by_clientid(clientid,last_update='14:48:10')
